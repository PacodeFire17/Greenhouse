...
#include <ti/devices/msp432p4xx/driverlib/driverlib.h>
    ...
    const uint8_t HUMIDIFIER_PIN = GPIO_PIN4;
    const uint8_t HUMIDIFIER_PORT_ = GPIO_PORT_P7;
    ...
    int main(void){
        ...
        // Output config
        GPIO_setAsOutputPin(HUMIDIFIER_PORT_, HUMIDIFIER_PIN);
        // Turn off all
        GPIO_setOutputLowOnPin(HUMIDIFIER_PORT_, HUMIDIFIER_PIN);
        printf("Initialized, waiting 5s\n");
        delay_ms(5000);


        // --- CYCLING 15 times on/off ---
        int i;
        for (i = 0; i < 15; i++ ) {
            printf("STARTING CYCLE\n");
